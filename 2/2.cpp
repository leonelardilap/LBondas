...
int main(void){
  ...
  //for(ix=0;ix<=Lx-2;ix++) densidad_80000[ix]=Ondas.getdensidad(ix);
  //Ondas.Imprimase("Ondas.dat",t);  
  //for(ix=0;ix<=Lx-2;ix++) densidad_80000[ix]=Ondas.getdensidad(ix);
  //for(ix=0;ix<=Lx-2;ix++) cout << ix << " " << densidad_80000[ix] << endl;
  ...
  for(ix=0;ix<=Lx-2;ix++) cout <<ix<<" "<<densidad_min[ix]<<" "<<densidad_max[ix]<<endl;
